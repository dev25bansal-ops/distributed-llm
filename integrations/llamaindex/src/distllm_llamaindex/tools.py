"""DistLLM Tool provider — LlamaIndex Tool integration.

Provides DistLLM as a ``FunctionTool`` provider for use with LlamaIndex
agents. Exposes the API server's available tools and allows custom tool
definitions to be executed by the DistLLM backend.
"""
...
class DistLLMToolProvider:
    ...
    LlamaIndex-compatible ``FunctionTool`` instances.
    ...
    def get_tools(self) -> list[Any]:
        """Discover and return LlamaIndex-compatible tools from the API.
        ...
        for tool_def in registered:
            tool = FunctionTool.from_defaults(
                fn=self._make_api_call(tool_def),
