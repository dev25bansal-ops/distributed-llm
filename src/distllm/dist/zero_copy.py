...
class RDMAManager:
    ...
    def recv_rdma(self, peer: str, shape: tuple[int, ...], dtype: torch.dtype) -> torch.Tensor | None:
        if not self._available:
            return None
        try:
            # RDMA receive requires real implementation — returning zeros would
            # cause silent data corruption. Raise to fall back to other transports.
            raise NotImplementedError(
                "RDMA receive not implemented — use CUDA IPC (local) or NCCL (multi-GPU) instead"
            )
        except Exception as e:
...
class ZeroCopyTransferEngine:
    ...
    def _select_backend(self, peer_is_local: bool, tensor: torch.Tensor) -> TransferBackend:
        ...
        # RDMA disabled — recv_rdma is a stub that would return garbage
