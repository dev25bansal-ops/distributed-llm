...
from distllm.dist.node_service import tensor_to_proto, tensor_from_proto, kv_cache_to_proto, kv_cache_from_proto
...
class LlamacppPipelineEngine:
    ...
    def run_pipeline(
        ...
        for idx, node_id in enumerate(self._node_order):
            ...
            current_hidden = tensor_from_proto(response.output, device="cpu")

            if response.kv_cache is not None:
                kv = kv_cache_from_proto(response.kv_cache, device="cpu")
                node_kv_caches[node_id] = kv.cache if hasattr(kv, 'cache') else kv
