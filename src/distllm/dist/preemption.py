...
class PreemptionPolicy:
    ...
    def should_preempt(
        ...
        if request_id is not None:
            violations = self.sla_tracker.get_violation_count(request_id)
            if violations > self.sla_tracker.max_violations:
                logger.info(f"Preemption trigger: SLA violations for {request_id} ({violations})")
                return True
