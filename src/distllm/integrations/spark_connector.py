...
# Optional: DistLLM MLflow integration (primary from new location,
# fallback to legacy integrations/ path for backward compatibility)
try:
    ...
    try:
        from integrations.mlflow_tracking import MLflowIntegration  # type: ignore[no-redef]
...
# Auto-tuning thresholds (milliseconds per request)
_TUNE_LATENCY_THRESHOLD_HIGH = 2000.0  # Reduce batch if avg > this
_TUNE_LATENCY_THRESHOLD_LOW = 200.0  # Increase batch if avg < this
_TUNE_CHECK_INTERVAL_SECONDS = 10.0  # Min seconds between re-tuning checks
_TUNE_MIN_SAMPLES = 10  # Min latency samples before re-tuning
...
class DistLLMSparkTransformer:
    ...
    def __init__(
        ...
        # Auto-tuning runtime state
        self._last_tune_check: float = time.time()
    ...
    def transform_batch(
        ...
        Uses ``toLocalIterator()`` to avoid pulling all data into driver
        memory (fixes OOM on large DataFrames).
        """
        if not _SPARK_AVAILABLE:
            raise ImportError("PySpark is required to use DistLLMSparkTransformer.")
        ...
        # Auto-tune at the start if not already tuned.
        if not self._tuned:
            self._auto_tune_batch_size(df, input_col, resolved_model)
            batch_size = self._actual_batch_size

        # Accumulate prompts partition-by-partition via toLocalIterator.
        # This avoids OOM on large DataFrames by never materialising the
        # full column of prompts in driver memory at once.
        partition_buffer: list[str] = []

        for row in df.toLocalIterator():
            partition_buffer.append(row[input_col])

            # Drain buffer in batch_size chunks.
            while len(partition_buffer) >= batch_size:
                batch = partition_buffer[:batch_size]
                partition_buffer = partition_buffer[batch_size:]

                output_texts, batch_retries, batch_tokens, batch_latencies = (
                    self._process_one_batch(client, batch, resolved_model)
                )
                ...
                for prompt, text in zip(batch, output_texts):
                    results.append((prompt, text))

        # Process remaining prompts in the final (partial) buffer.
        if partition_buffer:
            output_texts, batch_retries, batch_tokens, batch_latencies = (
                self._process_one_batch(client, partition_buffer, resolved_model)
            )
            retries += batch_retries
            if batch_latencies:
                all_latencies.extend(batch_latencies)
            if batch_tokens is not None:
                total_tokens += batch_tokens if batch_tokens else 0
                batches += 1
            else:
                errors += 1

            for prompt, text in zip(partition_buffer, output_texts):
                results.append((prompt, text))
        ...
        if metrics:
            ...
            avg_tok = total_tokens / max(len(results), 1)
            result_df.batch_metrics = BatchMetrics(  # type: ignore[attr-defined]
                ...
                p95_latency_ms=p95,
                p99_latency_ms=p99,
                throughput_rows_per_sec=len(results) / (elapsed_ms / 1000) if elapsed_ms > 0 else 0.0,
    ...
    def _process_one_batch(
        ...
        batch: list[str],
        model: str,
    ) -> tuple[list[str], int, int | None, list[float]]:
        """Send a single batch to the API with retries.

        Returns
        -------
        (output_texts, num_retries, total_tokens, latencies_ms)
        """
        retries = 0
        for attempt in range(self._max_retries + 1):
            try:
                result = self._call_batch(client, batch, model)
                self._latency_samples.extend(result.latencies_ms)
                self._maybe_tune_batch_size()
                return (result.outputs, retries, result.total_tokens, result.latencies_ms)
            except Exception:
                if attempt < self._max_retries:
                    ...
                    logger.warning(
                        "Batch of size %d failed after %d retries",
                        len(batch),
                        self._max_retries,
                    )
                    return ([""] * len(batch), retries, None, [])
    ...
    def _call_batch_udf(
        ...
        Uses ``ThreadPoolExecutor`` to issue parallel HTTP requests for the
        prompts in this batch instead of sequential calls.
        """
        n = len(prompts)
        results: list[str] = [""] * n
        max_workers = min(n, 8)
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            ...
            for fut in as_completed(fut_to_idx):
                ...
                try:
                    ...
                    results[i] = texts[0] if texts else ""
                except Exception:
                    if any(
                        attempt < self._max_retries
                        for attempt in range(self._max_retries)
                    ):
                        # Re-raise so the outer retry loop handles it
                        raise
                    results[i] = ""
    ...
    def _call_batch(
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            fut_to_idx: dict[Any, tuple[int, float]] = {}
            for i, prompt in enumerate(prompts):
                t0 = time.time()
                fut = executor.submit(
                    client.chat_completions,
    ...
    def _maybe_tune_batch_size(self) -> None:
        """Adjust batch size based on recent response-time history.

        Called after each batch completes.  Uses a sliding window of recent
        per-request latency samples to decide whether to increase (fast
        responses) or decrease (slow responses) the batch size.

        Re-tuning runs at most every ``_TUNE_CHECK_INTERVAL_SECONDS``.
        """
        samples = self._latency_window()
        if len(samples) < _TUNE_MIN_SAMPLES:
            return

        now = time.time()
        if now - self._last_tune_check < _TUNE_CHECK_INTERVAL_SECONDS:
            return
        self._last_tune_check = now

        avg_latency = statistics.mean(samples)
        current_bs = self._actual_batch_size

        if avg_latency > _TUNE_LATENCY_THRESHOLD_HIGH and current_bs > _MIN_BATCH_SIZE:
            new_bs = max(_MIN_BATCH_SIZE, current_bs // 2)
            if new_bs != current_bs:
                logger.info(
                    "Reducing batch size %d -> %d (avg latency %.0f ms)",
                    current_bs,
                    new_bs,
                    avg_latency,
                )
                self._actual_batch_size = new_bs
        elif avg_latency < _TUNE_LATENCY_THRESHOLD_LOW and current_bs < _MAX_BATCH_SIZE:
            new_bs = min(_MAX_BATCH_SIZE, current_bs * 2)
            if new_bs != current_bs:
                logger.info(
                    "Increasing batch size %d -> %d (avg latency %.0f ms)",
                    current_bs,
                    new_bs,
                    avg_latency,
                )
    ...
    def _latency_window(self) -> list[float]:
        """Return the recent latency history (sliding window)."""
        return self._latency_samples[-_BATCH_SIZE_TUNE_SAMPLES:]
...
def _percentiles(values: list[float]) -> tuple[float, float, float]:
    ...
    n = len(sorted_vals)
    # Use nearest-rank method: multiply by (n-1) so that p=1.0 maps to the
    # last index, and p=0.0 maps to the first.
    return (
        sorted_vals[max(0, min(n - 1, int((n - 1) * 0.50)))],
        sorted_vals[max(0, min(n - 1, int((n - 1) * 0.95)))],
        sorted_vals[max(0, min(n - 1, int((n - 1) * 0.99)))],
    )
