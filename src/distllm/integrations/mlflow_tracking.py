...
class MLflowIntegration:
    ...
    def log_run(
        ...
        # Try to set the experiment; create it if it doesn't exist.
        try:
    ...
    def log_model(
        ...
        **kwargs: Any,
        ...
        ``torch.nn.Module``, then checks for HuggingFace ``PreTrainedModel``,
        and falls back to ``mlflow.pyfunc.log_model`` for arbitrary callables.
        ...
        **kwargs:
        ...
        # Generic fallback via pyfunc (works for arbitrary callables / pickle).
        mlflow.pyfunc.log_model(model, artifact_path, **kwargs)
    ...
    def get_runs(
        ...
        if experiment_obj:
            ...
            if active is not None:
                experiment_ids = [active.info.experiment_id]
            else:
                logger.warning("No active run and no experiment specified")
                return []
    ...
    def _maybe_autolog(self) -> None:
        ...
        Thread-safe: uses a threading.Lock to prevent concurrent
        initialisation.
        """
        if not _MLFLOW_AVAILABLE or not self._autolog:
            return
        if self._autologged:
            return
...
class _RunContext:
    ...
    Marks the run as ``FAILED`` when an exception propagates.
    """
