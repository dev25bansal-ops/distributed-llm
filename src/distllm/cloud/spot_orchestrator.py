"""GPU Spot Market -- Multi-Provider Spot Instance Manager.
...
Typical usage::
    ...
    from distllm.cloud.spot_orchestrator import GPUSpotMarket, SpotOrchestrator
...
def normalize_gpu_name(name: str) -> str:
    """Strip non-canonical vendor prefixes from *name* so lookups match.

    Examples
    --------
    "NVIDIA GeForce RTX 4090" -> "NVIDIA RTX 4090"
    "NVIDIA RTX 4090"        -> "NVIDIA RTX 4090" (unchanged)
    """
    return name.replace("GeForce ", "")
...
#
# Note: names use the canonical "NVIDIA RTX" prefix (no "GeForce").
#       :func:`normalize_gpu_name` is applied before lookup so provider
#       responses that include "GeForce" still match.
# ---------------------------------------------------------------------------
...
class _PriceTracker:
    ...
    def record_price(self, gpu_type: str, price: float) -> None:
        ...
        key = normalize_gpu_name(gpu_type)
        if key not in self._prices:
            self._prices[key] = []
        window = self._prices[key]
        window.append(price)
    ...
    def recent_prices(self, gpu_type: str) -> list[float]:
        ...
        return list(self._prices.get(normalize_gpu_name(gpu_type), []))
    ...
    def suggest_bid(
        ...
        """
        gpu_key = normalize_gpu_name(gpu_type)
        on_demand = on_demand_price or GPU_ON_DEMAND_REFERENCE.get(gpu_key)
        if on_demand is None or on_demand <= 0:
            # Try the historical average as a fallback reference
            avg = self.moving_average(gpu_key)
            if avg:
                return round(avg, 3)
            return None

        # Candidate from historical P75 (more robust than mean)
        candidate = self.p75(gpu_key)
    ...
    def describe(self, gpu_type: str) -> dict[str, Any]:
        ...
        gpu_key = normalize_gpu_name(gpu_type)
        prices = self._prices.get(gpu_key, [])
        on_demand = GPU_ON_DEMAND_REFERENCE.get(gpu_key)
        return {
            "gpu_type": gpu_type,
            "observations": len(prices),
            "moving_avg": self.moving_average(gpu_key),
            "p75": self.p75(gpu_key),
            "on_demand_reference": on_demand,
            "recommended_bid": self.suggest_bid(gpu_key, on_demand),
        }


# ---------------------------------------------------------------------------
# ProviderProtocol  -- type-safe interface for provider implementations
# ---------------------------------------------------------------------------


class ProviderProtocol(Protocol):
    """Protocol defining the interface for GPU marketplace providers.

    Every provider (*_RunPodProvider*, *_VastProvider*, *_SaladProvider*)
    conforms to this protocol for type-safe dispatch.
    """
    ...
    def list_instances(
        ...
        """Query the marketplace for available GPU instances."""
        ...

    def bid(self, config: dict[str, Any]) -> BidResult:
        """Place a spot bid / rental request."""
        ...

    def cancel_bid(self, instance_id: str) -> bool:
        """Terminate a running instance by *instance_id*."""
        ...
...
# ---------------------------------------------------------------------------
# Base HTTP client helper  (with retry)
# ---------------------------------------------------------------------------

_HTTP_TIMEOUT: float = 30.0
_MAX_RETRIES: int = 3
_RETRY_BACKOFF: float = 1.0  # seconds, doubles each attempt
...
def _http_get(
    ...
    """Wrapper over ``httpx.get`` with retry and backoff."""
    import httpx
    ...
    for attempt in range(1, _MAX_RETRIES + 1):
        try:
            ...
            if attempt < _MAX_RETRIES:
                ...
                logger.debug("HTTP GET {!r} attempt {}/{} failed, retrying in {:.1f}s: {}",
...
def _http_post(
    ...
    """Wrapper over ``httpx.post`` with retry and backoff."""
    import httpx
    ...
    for attempt in range(1, _MAX_RETRIES + 1):
        try:
            ...
            if attempt < _MAX_RETRIES:
                delay = _RETRY_BACKOFF * (2 ** (attempt - 1))
                logger.debug("HTTP POST {!r} attempt {}/{} failed, retrying in {:.1f}s: {}",
                             url, attempt, _MAX_RETRIES, delay, exc)
                time.sleep(delay)
    raise last_exc  # type: ignore[misc]
...
def _http_delete(
    ...
    """Wrapper over ``httpx.delete`` with retry and backoff."""
    import httpx
    ...
    for attempt in range(1, _MAX_RETRIES + 1):
        try:
            ...
            if attempt < _MAX_RETRIES:
                delay = _RETRY_BACKOFF * (2 ** (attempt - 1))
                logger.debug("HTTP DELETE {!r} attempt {}/{} failed, retrying in {:.1f}s: {}",
                             url, attempt, _MAX_RETRIES, delay, exc)
                time.sleep(delay)
    raise last_exc  # type: ignore[misc]
...
class _VastProvider:
    ...
    def list_instances(
        ...
        try:
            # API key passed via Authorization header (not query param)
            data = _http_get(f"{self._BASE}/bulk_info/", headers=self._headers)
        except Exception as exc:
        ...
        for offer in offers:
            ...
            # NOTE: Vast API field "storage_cost" is the storage cost in USD,
            # NOT the disk size in GB. Use "disk_size" for the actual GB value.
            results.append(
                GPUInstance(
                    ...
                    storage_gb=float(offer.get("disk_size", 0)),  # fixed: was storage_cost->cost in USD
                )
            )
        return results
...
class GPUSpotMarket:
    ...
    def _get_provider(self, provider: str) -> ProviderProtocol:
        if self._closed:
    ...
    def bid(self, provider: str, config: dict[str, Any]) -> BidResult:
        ...
        if "max_price" not in config or not config.get("max_price"):
            gpu_type = config.get("gpu_type", "")
            on_demand = GPU_ON_DEMAND_REFERENCE.get(normalize_gpu_name(gpu_type))
