"""Multi-node pipeline engine orchestrating llama.cpp-backed worker nodes.

Provides a drop-in replacement for VLLMPipelineEngine where each node
runs LlamacppNodeAdapter for lightweight GGUF inference. Supports
CPU, CUDA, AMD ROCm, and Apple Metal backends.
"""
...
class LlamacppPipelineEngine:
    """Coordinates multi-node pipeline forward passes using llama.cpp on each node.

    Mirrors VLLMPipelineEngine's topology and gRPC-based forward calls,
    but delegates to LlamacppNodeAdapter on each worker node.

    Args:
        node_order: Ordered list of node IDs for pipeline execution.
        nodes: Mapping of node ID to node configuration objects.
        timeout_s: gRPC timeout per forward pass in seconds.
    """
    ...
    def run_pipeline(
        ...
        Each node executes LlamacppNodeAdapter.forward() on hidden states
    ...
    def shutdown(self):
        logger.info("[LlamacppPipeline] Engine shut down")
