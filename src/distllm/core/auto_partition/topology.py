...
@dataclass
class LinkProfile:
    """Profiled characteristics of a network link between two nodes."""
    source: str
    target: str
...
@dataclass
class TopologyGraph:
    """Complete topology: nodes, links, per-node NUMA/GPU layout."""
...
class TopologyProber:
    """Profiles inter-node network topology.

    Measures latency and bandwidth between nodes to build a complete
    topology graph. Supports both intra-node (NVLink) and inter-node
    (InfiniBand/Ethernet) links.

    Usage:
        prober = TopologyProber()
        graph = await prober.probe(node_ids=["node-0", "node-1", ...])
    """
    ...
    async def probe(
        ...
        """Probe all inter-node links and build topology.
        ...
        Args:
            ...
            gpu_profiles: Optional per-node GPU profiles.

        Returns:
            Complete TopologyGraph with profiled links.
        """
        ...
        for i, src in enumerate(node_ids):
            for j, dst in enumerate(node_ids):
                ...
                # Intra-node GPUs also get NVLink where available

        graph.links = links
        return graph

    def probe_local_topology(self, num_gpus: int) -> list[LinkProfile]:
        """Probe intra-node GPU topology (NVLink detection).

        Args:
            num_gpus: Number of GPUs in the local node.

        Returns:
            List of LinkProfiles between GPU pairs.
        """
        links: list[LinkProfile] = []
    ...
    def _detect_intra_node_bandwidth(self, src: str, dst: str) -> float:
        try:
            from distllm.core.hybrid_parallel import HardwareProber
    ...
    def _measure_latency(self, src: str, dst: str) -> float:
        return 5.0  # intra-node is negligible
    ...
    async def _measure_latency_async(
        ...
        try:
            ...
            return 500.0  # 0.5ms typical for same-region
    ...
    def make_fallback_topology(
        ...
        """Create a fallback topology when probing is unavailable."""
