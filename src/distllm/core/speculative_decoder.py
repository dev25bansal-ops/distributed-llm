...
class SpeculativeDecoder:
    ...
    def generate_draft_tokens(
        ...
        if active_method == "ngram":
            ...
            return self._generate_medusa_drafts(target_logits, hidden_states)
        else:
    ...
    def generate_batch_draft_tokens(
        ...
        def _gen_one(i: int, input_ids: torch.Tensor) -> tuple:
            ...
            return i, draft_tokens, new_kv

        with ThreadPoolExecutor(max_workers=min(n, 4)) as executor:
            future_map = {executor.submit(_gen_one, i, t): i for i, t in enumerate(input_ids_list)}
            for future in as_completed(future_map):
                i, draft_tokens, new_kv = future.result()
