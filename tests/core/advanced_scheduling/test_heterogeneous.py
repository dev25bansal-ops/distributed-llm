...
class TestHeterogeneousBudgetComputer:
    ...
    def test_compute_budget_no_nodes_returns_base(self) -> None:
        computer = HeterogeneousBudgetComputer()
        budget = SimpleNamespace(max_prefill_tokens=4096)
        assert computer.compute_budget(budget) is budget
    ...
    def test_compute_budget_homogeneous_no_change(self) -> None:
        ...
        result = computer.compute_budget(budget)
    ...
    def test_compute_budget_zero_tflops_skipped(self) -> None:
        ...
        budget = SimpleNamespace(max_prefill_tokens=4096)
        result = computer.compute_budget(budget)
        # min_tflops only looks at > 0, so min=200, max=200 (0 is skipped in min),
        # ratio=1.0, no change
        assert result.max_prefill_tokens == 4096

    def test_compute_budget_all_zero_tflops(self) -> None:
        """When all nodes have zero TFLOPS, min() raises ValueError."""
        nodes = {
            "a": NodeCapabilityInfo(node_id="a", gpu_tflops=0.0),
        }
        computer = HeterogeneousBudgetComputer(nodes=nodes)
        ...
        with pytest.raises(ValueError, match="empty sequence|iterable argument is empty"):
            computer.compute_budget(budget)
    ...
    def test_compute_budget_mixed_zero_and_positive(self) -> None:
        ...
        computer = HeterogeneousBudgetComputer(nodes=nodes)
        budget = SimpleNamespace(max_prefill_tokens=5000)
